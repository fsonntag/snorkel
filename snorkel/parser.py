...
try:
    ...
    print("Warning, unable to load 'spaCy' module", file=sys.stderr)
...
class StanfordCoreNLPServer(Parser):
    ...
    def close(self):
        ...
        if self.process_group is not None:
            try:
                ...
                sys.stderr.write(
                    'Could not kill CoreNLP server (might already be closed)...\n'.format(self.process_group.pid))
    ...
    def parse(self, document, text, conn):
        ...
        '''
        if len(text.strip()) == 0:
            print("Warning, empty document {0} passed to CoreNLP".format(document.name if document else "?"),
                  file=sys.stderr)
            return
        ...
        for block in blocks:
            ...
            # Add full dependency tree parse to document meta
            if 'parse' in block and not block['parse'] == 'SENTENCE_SKIPPED_OR_UNPARSABLE' and document:
