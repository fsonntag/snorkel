# from https://github.com/kaniblu/pytorch-bilstmcrf
import torch
...
class CRF(nn.Module):
    ...
    def transition_score(self, labels, lens):
        ...
        print(trn.size(0))
        print(lbl_r.size())
        values = lbl_r.size() + trn.size(0)
        print(values)
        lbl_rexp = lbl_r.unsqueeze(-1).expand(*values)
        # Python 3.5 syntax
...
class BiLSTMCRF(nn.Module):
    ...
    def __init__(self, word_vocab, label_set, embeddings, hidden_dim,
                 dropout_prob=0.25):
        super(BiLSTMCRF, self).__init__()
        ...
        self.crf = CRF(label_set)
        self.n_labels = n_labels = self.crf.n_labels

        self.embeddings = nn.Embedding(len(word_vocab), self.word_dim, padding_idx=0)
    ...
    def _run_rnn_packed(self, cell, x, x_lens, h=None):
        ...
        print(x_lens.data.numpy())
        print(x_lens.data.tolist())
        x_packed = R.pack_padded_sequence(x, x_lens.data.tolist(),
                                          batch_first=True)
        print("pack padded", x_packed)
    ...
    def _forward_bilstm(self, xs, lens):
        ...
        batch_size, n_feats, seq_len = xs.size()
        xs = Variable(xs.view(batch_size, -1))
        lens = Variable(lens)
        ...
        print("selu()", x.size())
        print("batch:", batch_size, "seqlen:", seq_len, "hidden:", self.hidden_dim)


        x = x.view(batch_size, seq_len, self.hidden_dim)
        print(x.size())
        print("Running RNN packed")
