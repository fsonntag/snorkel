...
class BratAnnotator(object):
    ...
    def map_annotations(self, session, annotation_dir, candidates, binary=True, categorical=False,
        ...
        mapped_cands, missed, false_positives = [], [], []
        if binary:
            ...
            for span in brat_stable_ids:
                ...
                if categorical or span_entry.meta['type'] == self.candidate_class.__name__:
                    if span in candidate_stable_ids:
                        if categorical:
                            mapped_cands.append((candidate_stable_ids[span], span_entry.meta['type']))
                        else:
        ...
        with codecs.open(ann_filename, "rU", encoding=self.encoding) as fp:
            for line in fp:
                ...
                if anno_id_prefix == StandoffAnnotations.TEXT_BOUND_ID:
                    ...
                    spans = list(map(lambda x: map(int, x.split()), entity.lstrip(entity_type).split(";")))
...
def print_scores(ntp, nfp, ntn, nfn, title='Scores'):
    ...
    print(title)
    print("========================================")
    print("Pos. class accuracy: {:.2}".format(100 * pos_acc))
    print("Neg. class accuracy: {:.2}".format(100 * neg_acc))
    print("Precision            {:.2}".format(100 * prec))
    print("Recall               {:.2}".format(100 * rec))
    print("F1                   {:.2}".format(100 * f1))
    print("----------------------------------------")
